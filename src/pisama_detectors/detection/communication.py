...
class CommunicationBreakdownDetector:
    ...
    _ACTION_VERBS = frozenset({
        ...
        "backup", "restart", "restore", "compress", "resize",
        "increase", "decrease", "schedule", "export", "import",
        "renew", "trigger", "start", "stop",
    })
    ...
    _STOP_WORDS = frozenset({
        ...
        "should", "may", "might", "shall", "must", "need", "please",
        "me", "my", "your", "our", "their", "its", "been", "being",
        "i've", "i'll", "i'm", "there",
    })
    ...
    def _compute_intent_alignment(
        ...
        content_overlap = (
            len(content_req & content_resp) / len(content_req) if content_req else 0.0
        )
        ...
        negative_indicators = {"error", "fail", "cannot", "unabl", "refus", "sorri",
                               "don't", "can't", "doesn't", "couldn't"}
        if any(w in response_words for w in negative_indicators) or \
           any(self._stem(w) in negative_indicators for w in response_words):
            action_match *= 0.5
    ...
    def detect(
        ...
        substitution = (
            None if is_raw_trace
            else self._has_substitution_mismatch(request_words, response_words)
        )
        ...
        is_refusal = any(
            ...
            for marker in ("don't have", "cannot", "i am unable", "i'm unable",
                           "i need", "could you provide", "which ", "please provide")
        )
        ...
        completion_markers = (
            "completed", "complete.", "done.", "finished", "initiated",
            "triggered", "executed", "processed", "configured", "applied",
            "deployed", "installed", "launched", "delivered", "submitted",
            "scheduled", "sent to", "sent.", "has been", "have been",
            "will be", "is now", "are now", "up to date", "ready",
        )
        response_has_completion = any(
            m in receiver_response.lower() for m in completion_markers
        )
        request_stems = {self._stem(w) for w in request_words}
        response_stems = {self._stem(w) for w in response_words}
        stop_stems = {self._stem(w) for w in self._STOP_WORDS}
        ...
        action_confirmed = (
            response_has_completion
            and topic_overlap >= 0.30
            and not is_refusal
        )
        ...
        if not substitution and not is_raw_trace:
            contradictory_pair = self._has_contradictory_pair_mention(
                request_words, response_words
            )
        ...
        if (
            ...
            if (
                self._REFUSAL_RE.search(receiver_response)
                and len(receiver_response) < 400
            ):
                detected = False
                breakdown_type = None
            ...
            # Code function definition delivered — Python / JS artifact.
                ...
                overlap_content_req = {
                    w for w in request_stems
                    if w not in stop_stems and len(w) > 2
                }
                overlap_content_resp = {
                    w for w in response_stems
                    if w not in stop_stems and len(w) > 2
                }
                overlap_ratio = (
                    len(overlap_content_req & overlap_content_resp)
                    / len(overlap_content_req)
